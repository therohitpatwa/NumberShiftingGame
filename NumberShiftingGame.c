...
void createMatrix(int arr[][4])
{
    int n = 15;
    int num[n], i, j;
    for (i = 0; i < 15; i++) // These 1-15 number will be in th matrix for cheak your arranged matrix
        num[i] = i + 1;
...
void gameRule(int arr[][4])
{
    ...
    printf(ANSI_COLOR_YELLOW "---------------------\n" ANSI_COLOR_RESET);
    printf(ANSI_COLOR_RED"\n5.You can exit the game at any time by pressing 'E' or 'e' " ANSI_COLOR_RESET);
...
int shiftUp(int arr[][4])
{
    ...
    if (i == 0) // shifting up not possible
        return 0;

    // Successfully swapped two numbers !
    swap(&arr[i][j], &arr[i - 1][j]); //83
    return 1; // Success
}
...
int main()
{
    ...
    while (1)
    {
        createMatrix(arr); // calling function to create  matrix -17
        gameRule(arr);     // game rule function calling -179

        while (!winner(arr))  // checking for winning situation 62
        {
            system("cls");
